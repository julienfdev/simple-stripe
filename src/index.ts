...
export default class SimpleStripe {
  ...
  /**
   *
   * @param secretKey your test or live secret key (sk_***_***)
   * @param config an optional StripeConfig object
   */
  constructor(secretKey: string, config: Stripe.StripeConfig = { apiVersion: '2020-08-27' }) {
  ...
  public paymentIntent = {
    ...
    generate: async (
      amount: number,
      customer?: string,
      setupOffSession = false,
      currency = 'eur',
    ): Promise<SimplePaymentIntent> => {
      const additionnalOptions = {
        ...(setupOffSession && customer
  ...
  public customer = {
    ...
    flushDuplicatePaymentMethods: async (id: customer_id): Promise<number> => {
      ...
      const methodsToDetach = methods.filter((method, index) => {
        if (!index && methods.length > 1) {
          ...
          return (
            method.card!.fingerprint === methods[index + 1].card!.fingerprint &&
            method.created < methods[index + 1].created
          );
        } else {
    ...
    charge: async (
      ...
      amount: number,
      currency = 'eur',
    ): Promise<SimplePaymentIntent | void> => {
      try {
        const paymentIntent = await this.stripe.paymentIntents.create({
          amount,
          currency,
